...
2. **Build the Agent's Tools (Nodes):**
    - In `LangGraph`, each tool is a node. You will define the following nodes:
        - `retrieve_vector`: A node that calls your existing Qdrant vector search (`CodeRAG`).
        - `retrieve_graph`: A node that calls the new `GraphRetriever` from Phase 2.
        - `generate`: A node that takes the final set of documents and the question to generate an answer (similar to the final step in `CodeRAG`).
